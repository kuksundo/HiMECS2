...
uses
  ...
  NxScrollControl, NxInspector, UnitEngineBaseClass2, EngineConst; //EngineBaseClass_Old
  ...
  procedure EngineInfoInspector2Class_Old(AInspector: TNextInspector;
    AClass: TICEngine);
  procedure EngineInfoClass2Inspector_Old(AClass: TICEngine;
    AInspector: TNextInspector; AProjIndex: integer;
    AEvent: TNxItemNotifyEvent);

  procedure AssignOldToNewClass(AOld: TICEngine; ANew: TICEngine);
...
begin
  ...
  begin
    AClass.ICEnginePartCollect.Items[i].PartName :=
                            AInspector.Items[i*j+k].Caption;
    AClass.ICEnginePartCollect.Items[i].Maker :=
                            AInspector.Items[i*j+k+1].AsString;
    AClass.ICEnginePartCollect.Items[i].FFType :=
                            AInspector.Items[i*j+k+2].AsString;
    AClass.ICEnginePartCollect.Items[i].SerialNo :=
                            AInspector.Items[i*j+k+3].AsString;
  end;
end;
...
begin
  ...
  for i := 0 to AOld.ICEnginePartCollect.Count - 1 do
  begin
    LICEnginePartItem := ANew.ICEnginePartCollect.Add;
    LICEnginePartItem.PartName := AOld.ICEnginePartCollect.Items[i].PartName;
    LICEnginePartItem.Maker := AOld.ICEnginePartCollect.Items[i].Maker;
    LICEnginePartItem.FFType := AOld.ICEnginePartCollect.Items[i].FFType;
    LICEnginePartItem.SerialNo := AOld.ICEnginePartCollect.Items[i].SerialNo;
  end;
end;
