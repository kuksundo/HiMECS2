...
{1. DSiGetProcessWindow함수를 이용하여 Window Handle을 가져올때 Main Form Handle이 아닌 문제 발생:
 ...
 3. FCode 04는 125개 쿼리 가능, FCode 02는 1000개 쿼리 가능
}
interface
...
type
  ...
  public
    ...
    FRecvBoolBuf: array[0..MAX_DIGITAL_DATA] of Bool;//TCP 사용시 Digital 데이터 수신값 저장됨
    ...
    //FSendCommandList로 부터 BlockNo가 시작되는 Param Item Index를 반환함
...
begin
  ...
  begin
    ...
    //ParamNo가 없는 데이타(Modbus Data)를 가져옴
  ...
  begin
    ...
    begin
      ...
      begin //Digital
        ...
        begin
          LBitMode := True; //Data는 Digital인데 FC는 3,4인 경우 한개의 Word 데이터를 16등분 해야 함
          LCount := LCount * 16;
          SetLength(EventData.InpDataBuf, LCount);
