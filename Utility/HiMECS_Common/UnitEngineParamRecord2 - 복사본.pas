...
const
  ...
  DF_AVAT2_PARAM_BASE_DB_NAME = 'Avat2ParamBase.sqlite';
...
function InitEngineParamClient2(AEngineParamDBName: string; var AEngineParamModel: TSQLModel):TRestClientDB;
function CreateEngineParamModel: TSQLModel;
procedure DestroyEngineParamClient(AEngineParamDB: TRestClientDB; AEngineParamModel: TSQLModel); overload;
...
begin
  ...
  g_EngineParamDB:= TRestClientDB.Create(EngineParamModel, CreateEngineParamModel,
    LStr, TRestServerDB);
  g_EngineParamDB.Server.CreateMissingTables;
end;

function InitEngineParamClient2(AEngineParamDBName: string;
  var AEngineParamModel: TSQLModel):TRestClientDB;
var
  LStr: string;
begin
  if AEngineParamDBName = '' then
  ...
  Result:= TRestClientDB.Create(AEngineParamModel, CreateEngineParamModel,
    LStr, TRestServerDB);
...
function CreateEngineParamModel: TSQLModel;
begin
  result := TSQLModel.Create([TEngineParamRecord, TPLCInfoRec4Avat2, TDiscreteValue4Avat2, TEngineMultiStateRecord, TParamDescBase4AVAT2, TEncryptedRegInfo]);
end;
...
procedure DestroyEngineParamClient(AEngineParamDB: TRestClientDB; AEngineParamModel: TSQLModel);
begin
...
begin
  ...
  g_UITextPoolDB:= TRestClientDB.Create(UITextPoolModel, CreateUITextPoolModel,
    LStr, TRestServerDB);
  g_UITextPoolDB.Server.CreateMissingTables;
end;
...
begin
  ...
  g_ParamNDescriptorDB:= TRestClientDB.Create(ParamNDescriptorModel, CreateParamNDescriptorModel,
    LStr, TRestServerDB);
  g_ParamNDescriptorDB.Server.CreateMissingTables;
end;
...
begin
  ...
  g_ParamBaseDB:= TRestClientDB.Create(ParamBaseModel, CreateParamBaseModel,
    LStr, TRestServerDB);
  g_ParamBaseDB.Server.CreateMissingTables;
end;
...
begin
  ...
  g_Param4AVAT2DB:= TRestClientDB.Create(Param4AVAT2Model, CreateParam4AVAT2Model,
    LStr, TRestServerDB);
  g_Param4AVAT2DB.Server.CreateMissingTables;
end;
