unit UnitEngineParameterClass2;
{
...
type
  TEngParamListItemKind = (eplikNull, eplikModbus, eplikSensorType, eplikParameter, eplikFinal);
  ...
  private
    FTagName,
    FAddress,
    FStateDesc: string; //State 이름
    FStateValue: integer; //통신으로 수신하는 State Value
    FStateMeaning: string;//화면에 표시되는 State 상태
    FStateMemo: string;//State 상태 설명
  public
...
//    function AssignFromJsonArray(const AJsonArray: string): integer;
  ...
  published
    ...
    property StateMemo: string read FStateMemo write FStateMemo;
  end;
...
//  TMultiStateItem = class;
  ...
  public
   ...
   constructor Create(AOwner: TComponent);
    ...
    procedure Assign(Source: TEngineParameter);
    ...
    function LoadFromJSONArray2(AJsonArray: TRawUTF8DynArray; AIsAdd: Boolean=False): integer;
    function LoadFromJSONArrayOfMultiState(AJsonArray: RawUTF8; AIsAdd: Boolean=False): integer;
    function LoadFromSqliteFile(ADBFileName: string; AEngParamKind: TEngParamListItemKind = eplikNull): integer; overload;
    function LoadFromSqliteFile(ADB: TRestClientDB; AEngParamKind: TEngParamListItemKind = eplikNull): integer; overload;
    function SaveToSqliteFile(ADBFileName: string; AItemIndex: integer=-1): integer;
    function LoadFromSqliteFile4Secure(ADBFileName: string; AEngParamKind: TEngParamListItemKind = eplikNull): integer;
...
uses UnitRttiUtil2, UnitStringUtil, UnitEncryptedRegInfo2, UnitExcelUtil;

{ TEngineParameter }

procedure TEngineParameter.Assign(Source: TEngineParameter);
var
  i: integer;
begin
...
class function TEngineParameter.GetVarArrayFromEPColl4Excel(
  AEngParamColl: TEngineParameterCollect; ARowCount,
  AColCount: integer): variant;
var
...
begin
  ...
  LDocData.InitJSON(AJsonArray);//, [dvoAllowDoubleValue]
//  LDocData.InitArrayFromResults(AJsonArray);
...
function TEngineParameter.LoadFromJSONArray2(AJsonArray: TRawUTF8DynArray;
  AIsAdd: Boolean): integer;
var
  LEngineParameterItem: TEngineParameterItem;
  LVar: variant;
...
var
  ...
  LEngParamListAry: TRawUTF8DynArray;
  LSrcDB: TRestClientDB;
  LSrcModel: TSQLModel;
...
begin
  ...
  Result := LoadFromSqliteFile(LSrcDB, AEngParamKind);

  if LIsNewDB then
    DestroyEngineParamClient(LSrcDB, LSrcModel);
end;

function TEngineParameter.LoadFromSqliteFile(ADB: TRestClientDB; AEngParamKind: TEngParamListItemKind = eplikNull): integer;
var
  LEngParamListAry: TRawUTF8DynArray;
  LEngParamList: RawUtf8;
begin
  case AEngParamKind of
    eplikModbus: LEngParamListAry := GetEngParamList2JSONArrayFromAddress2(ADB);
    else
//      LEngParamListAry := GetEngParamList2JSONArrayFromSensorType2(ADB);
      LEngParamListAry := GetEngParamList2JSONArrayFromParamNo('', ADB);
  end;
//
//  LoadFromJSONArray(LEngParamList);
  LoadFromJSONArray2(LEngParamListAry);
  LEngParamList := GetEngineMultiStateList2JSONArrayFromSqlite(ADB);
...
var
  LOrmModel: TOrmModel;
  LRestClientDB: TRestClientDB;
begin
  LRestClientDB := InitEngineParamClient2(ADBFileName, LOrmModel);

  try
    Result := CompareProcessorInfoBetweenReginfo(LRestClientDB);
//    Result := CompareProcessorInfoBetweenReginfo(g_EngineParamDB);

    if Result = 1 then
      Result := LoadFromSqliteFile(LRestClientDB, AEngParamKind);
  finally
    LRestClientDB.Free;
    LOrmModel.Free;
